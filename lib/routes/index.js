...
var connecting_words = ["is", "and" , "with", "the","The", "of", "at", "in","a","for","an"]
...
{
   for (var i =0; i<connecting_words.length;i++)
   {
	   if(word == connecting_words[i])  {
...
router.get('/programs/morelikethis', function(req, res) {
    ...
    Provider.getProgramInfo(collectionName, req.query.pgmId, function (result) {
        ...
        for (var a =0; a<8; a++)
	  console.log('prog name ='+result[a].title+ 'pref value is '+result[a].pref);
