...
router.get('/morelikethis/results', function(req, res) {

    var collectionName = req.app.get('epgCollection');
    var collectionName = 'epg_collection';

    // Get userStartTime, userEndTime from the req object
    var pgmStartTime = req.query.pgmStartTime,
        pgmEndTime = req.query.pgmEndTime,
        cast= req.query.cast,
    ...
    Provider.getMoreLikeThisPrograms(collectionName, pgmStartTime, pgmEndTime,cast,genre,title, function (result) {
        for( var j = 0; j < result.length; j++){

            var pref;
            if (result[j].cast==cast && result[j].genre==genre){
                result[j].pref="2";
            } else if (result[j].cast==cast || result[j].genre==genre || result[j].title==title){
                result[j].pref="1";
            } else {
                result[j].pref="0";
            }
